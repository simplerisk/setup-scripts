...
# ── Cron job ─────────────────────────────────────────────────────────────────
echo "--- Cron ---"
check "cron.d entry for the SimpleRisk backup was removed" \
    test ! -f /etc/cron.d/simplerisk
...
if grep -qi ubuntu /etc/os-release 2>/dev/null || grep -qi debian /etc/os-release 2>/dev/null; then
    ...
    echo "--- Packages (openSUSE/SLES) ---"
    check "apache2 package is removed" \
        bash -c "! rpm -q apache2"
    check "mysql-community-server package is removed" \
        bash -c "! rpm -q mysql-community-server"
    check "PHP packages are removed" \
        bash -c "! rpm -qa 'php8*' | grep -q ."

    echo "--- Services (openSUSE/SLES) ---"
    check "Apache is not running"                        bash -c "! pgrep -x httpd-prefork > /dev/null"
    check "MySQL is not running"                          bash -c "! mysqladmin ping --silent 2>/dev/null"

    echo "--- Config files (openSUSE/SLES) ---"
    check "simplerisk Apache vhost config was removed" \
        test ! -f /etc/apache2/vhosts.d/simplerisk.conf
    check "simplerisk Apache SSL vhost config was removed" \
        test ! -f /etc/apache2/vhosts.d/ssl.conf
